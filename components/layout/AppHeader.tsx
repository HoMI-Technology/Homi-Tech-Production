            ...
            <div ref={moreRef} className="relative">
              <button
                ...
                className={`flex items-center gap-1 rounded-lg px-3 py-2 text-sm transition-colors ${
                  MORE.some((m) => isActivePath(pathname, m.href))
                    ? "text-cyan"
              ...
              {moreOpen && (
                <div role="menu" className="glass absolute right-0 mt-2 grid w-56 grid-cols-2 gap-0.5 p-2">
                  {MORE.map((item) => (
                    <Link
                      key={item.href}
                      href={item.href}
        ...
        menuContent={
          <>
            ...
            <button
              type="button"
              onClick={() => setPaletteOpen(true)}
              className="rounded-lg px-3 py-2 text-left text-sm text-dim hover:bg-slate-surface hover:text-light"
            >
              Jump to…
            </button>
            <div className="hairline my-1" />
