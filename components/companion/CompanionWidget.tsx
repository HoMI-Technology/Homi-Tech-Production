  ...
  return (
    <>
      <button
        ...
        className="companion-launcher compass-glow fixed right-6 z-[var(--z-menu)] flex h-14 w-14 items-center justify-center rounded-full border border-cyan/40 bg-navy-light/90 shadow-lg backdrop-blur bottom-[max(1.5rem,env(safe-area-inset-bottom,0px))]"
      >
        {/* v3 shell has no bottom tab bar — FAB sits on the safe-area floor. */}
        <HomieAvatar
      ...
      {open && (
        <div
          ...
          className="companion-sheet glass fixed right-4 z-[var(--z-overlay)] flex h-[min(70dvh,560px)] max-h-[calc(100dvh-env(safe-area-inset-top,0px)-8rem)] w-[min(380px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl shadow-2xl bottom-[max(5.5rem,calc(env(safe-area-inset-bottom,0px)+1.5rem))] sm:right-6"
        >
