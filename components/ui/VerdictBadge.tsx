...
export function VerdictBadge({ verdict, size = "md" }: { verdict: VerdictKey; size?: "sm" | "md" | "lg" }) {
  const meta = VERDICT_META[verdict];
  const pad = size === "lg" ? "px-6 py-3 text-lg" : size === "sm" ? "px-3 py-1 text-xs" : "px-4 py-1.5 text-sm";
  return (
    <span
