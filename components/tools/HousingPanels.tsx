  ...
  return (
    <ToolPanel desc={desc} seeded lender>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {tiers.map(({ tier, color }) => (
          ...
          >
            ...
            <p className="mt-3 score-numeral text-xl font-semibold tracking-[-0.01em] text-light">
            ...
            <div className="mt-3 space-y-1 border-t border-white/[0.06] pt-2.5">
              <p className="flex items-baseline justify-between text-xs text-dim">
                Monthly PITI
                <span className="score-numeral text-xs font-medium text-light">
                  {formatCurrency(tier.maxMonthlyHousing)}
                </span>
              </p>
              <p className="flex items-baseline justify-between text-xs text-dim">
                Loan amount
                <span className="score-numeral text-xs font-medium text-light">
                  {formatCurrency(tier.loanAmount)}
                </span>
              </p>
            </div>
          </div>
      ...
      <p className="mt-4 text-xs leading-relaxed text-dim">
        Protected keeps housing under 28% of gross income; Stretch reaches 33%; past the 36% Red
        Line one bad month becomes a crisis. Which tier a price lands in is the honest answer — not
...
export function AprPanel({ seeds, desc }: { seeds: LedgerSeeds; desc: string }) {
  ...
  return (
    <ToolPanel desc={desc} lender>
          ...
          return (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="eyebrow !text-light">{o.label}</span>
                {isBest && <ResultChip label="Lowest true cost" color={COLORS.cyan} />}
              </div>
              <div className="mt-3 grid grid-cols-3 gap-3">
                <NumberField
                  label="Note rate"
                  value={o.rate}
                  onChange={(v) => patchOffer(i, { rate: v })}
                  suffix="%"
                  step={0.125}
                  min={0}
                />
                <NumberField
                  label="Points"
                  value={o.points}
                  onChange={(v) => patchOffer(i, { points: v })}
                  step={0.125}
                  min={0}
                />
                <NumberField
                  label="Fees"
                  value={o.fees}
                  onChange={(v) => patchOffer(i, { fees: v })}
                  prefix="$"
                  step={100}
                  min={0}
                />
              ...
              {r && (
                <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 border-t border-white/[0.06] pt-2.5 sm:grid-cols-4">
                  <p className="text-xs text-dim">
                    Monthly{" "}
                    <span className="block score-numeral text-sm font-medium text-light">
                      {formatCurrency(r.monthly, { decimals: 2 })}
                    </span>
                  </p>
                  <p className="text-xs text-dim">
                    Upfront cost{" "}
                    <span className="block score-numeral text-sm font-medium text-light">
                      {formatCurrency(r.upfrontCost)}
                    </span>
                  </p>
                  <p className="text-xs text-dim">
                    True APR{" "}
                    <span
                      className="block score-numeral text-sm font-semibold"
                      style={{ color: isBest ? COLORS.cyan : COLORS.light }}
                    >
      ...
      <p className="mt-4 text-xs leading-relaxed text-dim">
        The note rate hides points and fees; the APR spreads them across the loan life. The lowest
        APR is the honest winner — not the lowest headline rate.
      </p>
    </ToolPanel>
  );
}
    ...
    <ToolPanel desc={desc} lender>
      ...
      <StatGrid>
        ...
        <Stat
          ...
          hint={
            r.breakEvenMonths !== null
              ? "Months until payment savings repay closing costs"
              : "The new loan costs more every month"
          }
        />
        <Stat
          label="Lifetime interest (current)"
          value={formatCurrency(r.currentLifetimeInterest)}
        />
        <Stat
  ...
  return (
    <ToolPanel desc={desc} lender>
      ...
      {mid && (
        <div className="mt-5">
          <ResultChip
            label={`${formatPercent(mid.equityPct * 100, 0)} equity today`}
            color={TIER_HEX.p90}
          />
        </div>
      )}
      <StatGrid>
        <Stat
          ...
          value={formatCurrency(mid?.equity ?? 0)}
          hint="Value minus balance — paper equity, not borrowable"
        />
        <Stat label="Current CLTV" value={formatPercent((mid?.currentCltv ?? 0) * 100, 1)} />
        <Stat
          label="Available at 85% CLTV"
          value={formatCurrency(mid?.availableLine ?? 0)}
          accent={COLORS.cyan}
        />
      </StatGrid>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-white/[0.06] bg-white/[0.02]">
              <th className="eyebrow px-4 py-2.5">CLTV cap</th>
              <th className="eyebrow px-4 py-2.5">Available line</th>
              <th className="eyebrow px-4 py-2.5">Interest-only / mo</th>
            </tr>
          </thead>
          <tbody>
            {tiers.map(({ cltv, result }) => (
              <tr key={cltv} className="border-b border-white/[0.04] last:border-0">
                <td className="px-4 py-2.5 score-numeral text-sm font-medium text-light">
                  {formatPercent(cltv * 100, 0)}
                </td>
                <td className="px-4 py-2.5 score-numeral text-sm font-medium text-light">
                  {formatCurrency(result.availableLine)}
                </td>
                <td className="px-4 py-2.5 score-numeral text-sm font-medium text-dim">
                  {formatCurrency(result.interestOnlyMonthly, { decimals: 2 })}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <p className="mt-4 text-xs leading-relaxed text-dim">
        Lenders cap combined loan-to-value, so the borrowable line is value × cap − balance — always
        less than paper equity. HELOC rates are typically variable; the interest-only figure assumes
        the full line is drawn.
      </p>
    </ToolPanel>
  );
}
...
export function LoanProgramsPanel({ seeds, desc }: { seeds: LedgerSeeds; desc: string }) {
  ...
  const cheapest = results.reduce(
    (best, r, i) => (r.monthlyTotal < results[best].monthlyTotal ? i : best),
    0,
  );
    ...
    <ToolPanel desc={desc} seeded lender>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {results.map((r, i) => (
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="eyebrow !text-light">{r.label}</span>
            ...
            <div className="mt-3 space-y-1 border-t border-white/[0.06] pt-2.5">
              ...
              <p className="flex items-baseline justify-between text-xs text-dim">
                ...
                <span className="score-numeral text-xs font-medium text-light">
                  {formatCurrency(r.upfrontFeeFinanced)}
                </span>
              </p>
              <p className="flex items-baseline justify-between text-xs text-dim">
                Monthly insurance
                <span className="score-numeral text-xs font-medium text-light">
                  {r.monthlyInsurance > 0
                    ? formatCurrency(r.monthlyInsurance, { decimals: 2 })
                    : "none"}
                </span>
              </p>
            </div>
            <p className="mt-2.5 text-xs leading-snug text-dim">{r.note}</p>
          </div>
        ))}
      </div>

      <p className="mt-4 text-xs leading-relaxed text-dim">
        Each program fits the typical profile for a different buyer: conventional rewards strong
        credit and 20% down, FHA lowers the barrier to entry, and VA serves eligible service members
        and veterans. Standard 2026 program rules, shown for education.
      </p>
    </ToolPanel>
  );
}
