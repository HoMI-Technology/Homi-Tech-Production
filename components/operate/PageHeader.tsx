    ...
    <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-2xs font-bold uppercase tracking-[0.14em] text-dim">{eyebrow}</p>
          {badge}
        </div>
        <h1 className="mt-1 type-h2 font-medium tracking-tight">{title}</h1>
        {description && (
        ...
        <div className="flex shrink-0 flex-wrap gap-2.5">
          {primaryAction && (
            <Link
              ...
              className={primaryAction.variant === "ghost" ? "btn btn-ghost" : "btn btn-primary"}
            >
              {primaryAction.label}
            </Link>
          )}
