...
import type { PillarKey } from '@/lib/score'
import {
...
import { downloadReceipt, issueReceipt } from '@/lib/planner-receipts'
import {
  fmtDayShort,
  fmtSignedUsd0,
  fmtUsd0,
...
import { useBehaviorNudges } from '@/components/planner/NudgeRail'
...
export default function OverviewCommand({
  ...
  const onIssueReceipt = () => {
    void (async () => {
      const receipt = await issueReceipt(planner.result)
      setReceiptToken(
        `homi_rcpt_${receipt.claims.verdict.toLowerCase()}_${receipt.claims.scoreBand}_${receipt.claims.deviceKeyId}`,
      )
