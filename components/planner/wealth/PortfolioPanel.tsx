...
import type { AssetClass, Holding, HoldingAccountKind } from "@/lib/planner/types";
...
function HoldingForm({
  ...
  const set = <K extends keyof HoldingFormState>(key: K, value: HoldingFormState[K]) =>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Field label="Account">
          <select
            ...
            onChange={(e) => set("accountKind", e.target.value as HoldingAccountKind)}
          >
    ...
    >
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <span className="font-display text-sm font-semibold text-cyan">{holding.symbol}</span>
        ...
        <p className="mt-0.5 text-xs text-dim">
          {holding.shares} sh · basis {formatCurrency(holding.costBasis, { decimals: 2 })} · as of{" "}
          {holding.asOf}
        </p>
      </div>
      ...
      <div className="h-[180px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip
              content={<ChartTooltip format={(n) => formatCurrency(n, { decimals: 2 })} />}
            />
          </PieChart>
        </ResponsiveContainer>
      </div>
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-base font-semibold text-light">Investment portfolio</h3>
            <p className="mt-0.5 max-w-md text-xs leading-relaxed text-dim">
              Holdings, allocation, and unrealized gain — marked prices, not live market feeds.
            </p>
          </div>
        </div>
