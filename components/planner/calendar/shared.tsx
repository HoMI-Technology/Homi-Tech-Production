...
export const CARD = "rounded-2xl border border-white/[0.06] bg-navyLight/60";
export const SUBCARD = "rounded-xl border border-white/[0.06] bg-white/[0.02]";

export const EYEBROW = "text-3xs font-medium uppercase tracking-[0.22em] text-cyan";
export const EYEBROW_DIM = "text-3xs font-medium uppercase tracking-[0.22em] text-dim";
...
export function EventDot({ kind, className }: { kind: DotKind; className?: string }) {
  return (
    <span className={cn("inline-block h-1.5 w-1.5 rounded-full", DOT_CLASS[kind], className)} />
  );
}
