...
const VERDICT_CHIP: Record<HousingLensResult["verdict"], { label: string; className: string }> = {
  blocked: {
    label: "BLOCKED",
...
export default function PlanHousing() {
  ...
  return (
    <section className="card-chrome card-hairline-top p-5 sm:p-6">
      ...
      <p className="mt-3 font-serif text-base italic leading-snug text-light/90">{lens.headline}</p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <PlanTile
          ...
          tone={
            lens.housingRatioPct > 45 ? "crimson" : lens.housingRatioPct > 36 ? "amber" : "emerald"
          }
        />
        ...
        <PlanTile
          label="RUNWAY HIT"
          value={lens.runwayHitMonths != null ? `~${lens.runwayHitMonths} mo` : "None"}
          hint={
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-3">
        ...
        <NumberField
          label="TAX+INS % / YR"
          suffix="%"
          value={taxInsuranceRatePct}
          onChange={setTaxInsuranceRatePct}
          step={0.05}
          min={0}
        />
      </div>
