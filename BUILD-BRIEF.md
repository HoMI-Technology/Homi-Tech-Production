...
**This budget is ENFORCED, not aspirational.** `lighthouserc.json` + `lighthouse-budget.json` encode the thresholds above (categories:performance ≥0.9, accessibility ≥0.95, CLS ≤0.1 error, LCP ≤2500 error, script/total transfer budgets). The `.github/workflows/lighthouse.yml` workflow runs `npx lhci autorun` (mobile emulation) on every PR that touches `app/**` or `components/**`, builds the app, and fails the check if a route breaks the budget. Run locally with `npm run lighthouse`. Caveats: (1) the "≤30KB added JS" figure is a review heuristic — the machine gate enforces absolute per-page transfer budgets instead; tighten `resource-summary:*:size` as the app slims. (2) LHCI runs only PUBLIC routes (landing, how-it-works, method, pricing, tools, tools/mortgage, shadow-score) because authed pages redirect to sign-in; to budget dashboards/results/journal, add a `collect.puppeteerScript` login step or point LHCI at a Vercel preview URL with a seeded session cookie (follow-up). (3) Set the repo variable `NEXT_PUBLIC_SUPABASE_ANON_KEY` (Settings → Secrets and variables → Actions → Variables) so both CI workflows can build.
