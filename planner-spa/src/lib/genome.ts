/* ------------------------------------------------------------------ */
/* HōMI Behavioral Genome — VERBATIM port of genome/dimensions.ts +    */
/* genome/constants.ts (GitHub HoMI-Technology/Homi-Tech-Production =  */
/* source of truth). Pure module, no side effects.                     */
/*                                                                     */
/* 9 decision-psychology dimensions. Each dimension has 2 questions    */
/* (agree <-> disagree, 1-7 slider). Some questions are reverse-scored */
/* so scale-gaming is harder and the dimension score reflects the      */
/* underlying trait rather than one phrasing.                          */
/*                                                                     */
/* Scoring: for each question, raw slider is 1-7. If reversed,         */
/* effective = 8 - raw. Dimension score (0-100) = average(effective)   */
/* mapped from [1,7] to [0,100]: ((avg - 1) / 6) * 100                 */
/* ------------------------------------------------------------------ */
...
/* -------- display constants (genome/constants.ts, adapted) -------- */

/** Pillar-colored rotation — cyan / emerald / gold. */
const COLORS = ['#22d3ee', TEMP_HEX.emerald, TEMP_HEX.yellow] as const
