...
/*                                                                     */
/* Ported from the reference planner impact.ts (lib audit #13: PORT,   */
/* wave 1), adapted to OUR canon engine (@/lib/score ScoreResult):     */
/*   - pillars read from result.pillars.<key>.total (canon 35/35/30),  */
/*     pillar display names sourced from PILLARS meta — not hardcoded  */
/*   - verdicts are the canon four-tier VerdictKey; headline labels    */
/*     come from VERDICT_META — not hardcoded                          */
/*   - hard-stops are HardStopKey[]; protective copy comes from        */
/*     HARD_STOP_MESSAGES — never reworded                             */
/*                                                                     */
/* Descriptions are operator-grade: what moved, why it matters, what   */
/* to do next. Protective honesty — never celebrate a drop as          */
/* failure; never inflate noise. Path steps: prefer progress framing   */
/* when the score is flat.                                             */
/* ------------------------------------------------------------------ */

import { HARD_STOP_MESSAGES, PILLARS, VERDICT_META } from '@/lib/score'
import type { PillarKey, ScoreResult, VerdictKey } from '@/lib/score'
...
export interface ScoreImpact {
  ...
  nextHint: string
  at: string
  /** Path completion ratio before/after (0–1). */
...
function pillarNarrative(
  ...
  ).map((key) => [PILLARS[key].label, deltas[key]])
...
export function buildScoreImpact(
  ...
  const beforeCodes = new Set(before.hardStops)
  const afterCodes = new Set(after.hardStops)
  ...
  if (hardStopsAdded > 0) {
    const added = after.hardStops.find((code) => !beforeCodes.has(code))
    parts.push(
      added
        ? `New stop: ${HARD_STOP_MESSAGES[added]}`
...
/** Map impact → lightweight snapshot for zustand persist. */
