...
import { daysUntil, todayISO } from '@/lib/planner/derived'
import type { StressAnalysis } from '@/lib/planner/stress'
import { HARD_STOP_MESSAGES } from '@/lib/score'
import type { ScoreResult } from '@/lib/score'
import type { Bill, PathSnapshot } from '@/lib/planner/types'
...
export function deriveBehaviorNudges(input: NudgeEngineInput): BehaviorNudge[] {
  ...
  if (input.assessment && input.assessment.hardStops.length > 0) {
    const hs = HARD_STOP_MESSAGES[input.assessment.hardStops[0]]
    nudges.push({
  ...
  if (input.stress.shouldSignal) {
    nudges.push({
      id: 'nudge-stress',
      kind:
        input.stress.reasonCode === 'spike' ? 'identity' : 'protect_decision',
