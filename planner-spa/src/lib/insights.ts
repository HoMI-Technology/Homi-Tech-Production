/* ------------------------------------------------------------------ */
/* Key insight + next steps — verbatim port of GitHub canon             */
/* lib/scoring/insights.ts (source of truth).                          */
/* Voice: precision empathy, calm authority. Never shaming.            */
/*                                                                     */
/* Local adaptations (field access only — copy is verbatim):           */
/*  - canon AssessmentResult.financial.total → ScoreResult             */
/*    .pillars.financial.total (PILLAR_MAX_POINTS unchanged)           */
/*  - canon sub-scores (f.emergencyFund …) → our factors[] pts lookup; */
/*    canon `debtToIncome`/`creditHealth`/`fomoCheck` keys map to our  */
/*    `dti`/`credit`/`fomo` factor keys                                */
/*  - canon e.singleRedistribution → passed in as an option (our       */
/*    AssessmentInputs.partnerAlignment === null)                      */
/* ------------------------------------------------------------------ */

import { PILLAR_MAX_POINTS } from '@/lib/score'
import type { PillarKey, ScoreResult } from '@/lib/score'

interface PillarScorePct {
  key: PillarKey
  name: string
...
function normalizedPillars(result: ScoreResult): PillarScorePct[] {
  return [
    {
      key: 'financial',
      name: 'Financial Reality',
      pct: Math.round((result.pillars.financial.total / PILLAR_MAX_POINTS.financial) * 100),
    },
    {
      key: 'emotional',
      name: 'Emotional Truth',
      pct: Math.round((result.pillars.emotional.total / PILLAR_MAX_POINTS.emotional) * 100),
    },
    {
      key: 'timing',
      name: 'Perfect Timing',
      pct: Math.round((result.pillars.timing.total / PILLAR_MAX_POINTS.timing) * 100),
    },
  ]
}
...
/** pts earned by one sub-factor (0 when the factor is absent). */
function factorPts(result: ScoreResult, pillar: PillarKey, factorKey: string): number {
  const factor = result.pillars[pillar].factors.find((f) => f.key === factorKey)
  return factor ? factor.pts : 0
}

export interface NextStepsOptions {
  /** True when the buyer is solo (canon `singleRedistribution`). */
  singleRedistribution?: boolean
}

/** Up to 5 context-aware next steps keyed off which pillars are below 60 / 80. */
export function generateNextSteps(result: ScoreResult, opts?: NextStepsOptions): string[] {
  const steps: string[] = []
  const fPct = (result.pillars.financial.total / PILLAR_MAX_POINTS.financial) * 100
  const ePct = (result.pillars.emotional.total / PILLAR_MAX_POINTS.emotional) * 100
  const tPct = (result.pillars.timing.total / PILLAR_MAX_POINTS.timing) * 100

  // Canon sub-score accessors mapped to our factor keys.
  const f = {
    emergencyFund: factorPts(result, 'financial', 'emergencyFund'),
    debtToIncome: factorPts(result, 'financial', 'dti'),
    downPayment: factorPts(result, 'financial', 'downPayment'),
    creditHealth: factorPts(result, 'financial', 'credit'),
  }
  const e = {
    fomoCheck: factorPts(result, 'emotional', 'fomo'),
    partnerAlignment: factorPts(result, 'emotional', 'partnerAlignment'),
    singleRedistribution: opts?.singleRedistribution ?? false,
  }
  const t = {
    savingsRate: factorPts(result, 'timing', 'savingsRate'),
    downPaymentProgress: factorPts(result, 'timing', 'downPaymentProgress'),
    timeHorizon: factorPts(result, 'timing', 'timeHorizon'),
  }
  ...
  if (ePct < 60) {
    ...
    if (e.fomoCheck < 4)
      steps.push('The urgency you feel is external, not internal. Give the decision 30 quiet days.')
