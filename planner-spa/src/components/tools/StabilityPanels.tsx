...
function defaultDebts(seeds: LedgerSeeds): Debt[] {
  ...
  return [
    { id: 'd-card', name: 'Credit card', balance: Math.round(total * 0.37), apr: 22.9, minPayment: 180 },
    { id: 'd-auto', name: 'Auto loan', balance: Math.round(total * 0.47), apr: 7.5, minPayment: 290 },
    { id: 'd-personal', name: 'Personal loan', balance: Math.round(total * 0.16), apr: 12.5, minPayment: 105 },
  ]
}
...
export function DebtPayoffPanel({ seeds, desc }: { seeds: LedgerSeeds; desc: string }) {
  ...
  const winner =
    interestSaved > 0
      ...
      : interestSaved < 0
        ? { label: `Snowball saves ${formatCurrency(Math.abs(interestSaved))}`, color: TIER_HEX.snowball }
        : { label: 'Identical interest cost', color: '#94a3b8' }
  ...
  return (
    <ToolPanel desc={desc} seeded>
      <div className="grid gap-3 md:grid-cols-3">
        {debts.map((d) => (
          <div key={d.id} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            <span className="text-label !text-light">{d.name}</span>
            <div className="mt-3 grid grid-cols-3 gap-2">
              <NumberField label="Balance" value={d.balance} onChange={(v) => patchDebt(d.id, { balance: v })} prefix="$" step={100} min={0} />
              <NumberField label="APR" value={d.apr} onChange={(v) => patchDebt(d.id, { apr: v })} suffix="%" step={0.1} min={0} />
              <NumberField label="Min pay" value={d.minPayment} onChange={(v) => patchDebt(d.id, { minPayment: v })} prefix="$" step={5} min={0} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 max-w-xs">
        <NumberField label="Extra monthly payment" value={extraMonthly} onChange={setExtraMonthly} prefix="$" step={25} min={0} />
      </div>
...
export function BlindBudgetPanel({ seeds, desc }: { seeds: LedgerSeeds; desc: string }) {
  ...
  return (
    <ToolPanel desc={desc} seeded>
      <InputGrid>
        <NumberField label="Monthly income (low)" value={incomeLow} onChange={setIncomeLow} prefix="$" step={100} min={0} />
        <NumberField label="Monthly income (high)" value={incomeHigh} onChange={setIncomeHigh} prefix="$" step={100} min={0} />
        <NumberField label="Fixed costs (low)" value={fixedLow} onChange={setFixedLow} prefix="$" step={50} min={0} />
        <NumberField label="Fixed costs (high)" value={fixedHigh} onChange={setFixedHigh} prefix="$" step={50} min={0} />
        <NumberField label="Savings (low)" value={savingsLow} onChange={setSavingsLow} prefix="$" step={250} min={0} />
        <NumberField label="Savings (high)" value={savingsHigh} onChange={setSavingsHigh} prefix="$" step={250} min={0} />
      </InputGrid>
