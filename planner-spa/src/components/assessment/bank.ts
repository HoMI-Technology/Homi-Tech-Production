...
// supabase/migrations/00006_seed_question_bank.sql).
// =============================================================================
...
export const QUESTION_BANK: Question[] = [
  ...
  // F9: Pre-approval status /* brand-ok: verbatim canon question id/title — domain term in a neutral survey question, not marketing copy */
  {
    id: 'fin_preapproval',
    dimension: 'financial',
...
// ---------------------------------------------------------------------------
// Flow sequencing (adapted from canon lib/questions/flow.ts — local-only:
// the decision picker and conflict-referral/deadline steps are canon-optional;
// this app serves the home_buying decision type only)
// ---------------------------------------------------------------------------
...
/** Act intro copy — verbatim from canon lib/questions/flow.ts PILLAR_INTRO. */
export const PILLAR_INTRO_COPY: Record<Dimension, { question: string; description: string }> = {
  financial: {
...
/**
 * Builds the ordered step list from the canonical question bank:
 * pillar intro → one step per bank question, per dimension, in
 * DIMENSION_ORDER, then the review step.
 */
export function buildAssessmentFlow(decisionType: string = 'home_buying'): FlowStep[] {
...
// ---------------------------------------------------------------------------
// Per-question scoring — NEVER re-derived: evaluated strictly from each
// question's own scoring_function (option maps, scales, thresholds) and
// weight exactly as defined in the bank above.
// ---------------------------------------------------------------------------

/**
 * Score a single response 0–100 using ONLY the question's bank-defined
 * scoring_function. Returns null when no usable response is present.
 */
export function scoreResponse(question: Question, value: ResponseValue | undefined): number | null {
  if (value === undefined) return null;
  const sf = question.scoring_function;
  switch (sf.type) {
    case 'option_map': {
      if (typeof value !== 'string') return null;
      const pts = sf.scores[value];
      return typeof pts === 'number' ? pts : null;
    }
    case 'slider_direct': {
      if (typeof value !== 'number' || !Number.isFinite(value)) return null;
      const v = Math.min(sf.max, Math.max(sf.min, value));
      const t = sf.max === sf.min ? 1 : (v - sf.min) / (sf.max - sf.min);
      return Math.round(sf.output_min + t * (sf.output_max - sf.output_min));
    }
    case 'linear_scale': {
      if (typeof value !== 'number' || !Number.isFinite(value)) return null;
      if (value >= sf.optimal_min) return 100;
      const span = sf.optimal_min - sf.min;
      if (span <= 0) return 100;
      return Math.round(Math.min(100, Math.max(0, ((value - sf.min) / span) * 100)));
    }
    case 'inverse_scale': {
      if (typeof value !== 'number' || !Number.isFinite(value)) return null;
      if (value <= sf.optimal_max) return 100;
      const span = sf.max - sf.optimal_max;
      if (span <= 0) return 0;
      return Math.round(Math.min(100, Math.max(0, (1 - (value - sf.optimal_max) / span) * 100)));
    }
    case 'threshold': {
      if (typeof value !== 'number' || !Number.isFinite(value)) return null;
      for (const entry of sf.thresholds) {
        if (value <= entry.max) return entry.score;
      }
      return sf.thresholds[sf.thresholds.length - 1]?.score ?? null;
    }
    default:
      return null;
  }
...
/** Weighted points for review display: bank score × bank weight. */
export function weightedPoints(question: Question, value: ResponseValue | undefined): number | null {
  const s = scoreResponse(question, value);
  return s === null ? null : Math.round(s * question.weight * 10) / 10;
}

/** Human-readable rendering of a response for the review screen. */
export function formatAnswer(question: Question, value: ResponseValue | undefined): string {
  ...
  if (typeof value === 'number') {
    const sf = question.scoring_function;
    const unit = 'unit' in sf ? sf.unit : undefined;
    if (unit === 'usd') return `$${value.toLocaleString('en-US')}`;
    return `${value} / ${(question.options as SliderConfig | null)?.max ?? 10}`;
  }
  return String(value);
}
