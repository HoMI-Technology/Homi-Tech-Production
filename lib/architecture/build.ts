...
export const ARCHITECTURE_DB_TABLES: ArchitectureDbTable[] = [
  ...
  },
  {
    ...
    rls: true,
    category: "user",
    description: "Longitudinal outcome checkpoints (taxonomy + structured fields)",
  },
  {
    name: "assessment_outcome_baselines",
    columns: ["assessment_id", "user_id", "schema_version"],
    rls: true,
    category: "user",
    description: "Immutable T0 research baseline; insert-only for owners",
  },
  {
    name: "outcome_survey_events",
    columns: ["id", "survey_id", "event_type"],
    rls: true,
    category: "user",
    description: "Survey contact funnel events without answer payloads",
  },
  {
    name: "payments",
...
const API_PURPOSES: Record<string, { purpose: string; method: string }> = {
  ...
  outcomes: { purpose: "Outcome survey capture (no scoring writes)", method: "POST" },
