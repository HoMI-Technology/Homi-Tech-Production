...
export function analyzeStress(checkins: DailyCheckin[]): StressAnalysis {
  ...
  if (values.length < 3) {
    ...
    nudge = "Protect the next decision: pause big commitments until the slope flattens for 3 days.";
  } else if (vol >= 2.2 && m >= 4) {
    trend = "volatile";
    ...
    nudge = "Pick a fixed check-in time and one weekly money ritual to damp the swing.";
  } else if (m >= 7) {
    trend = "elevated";
    ...
    nudge = "Lower the floor: one open loop (bill, path step, or conversation) closed today.";
