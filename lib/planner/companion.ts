/* ------------------------------------------------------------------ */
/* Rule-based HōMI Companion — deterministic fallback voice ported     */
/* from the reference planner companion.ts, wired to a live budget     */
/* finance context. Pure module: no store imports.                     */
/* Your homie, not your banker. Never financial advice.                */
/*                                                                     */
/* Two deliberate adaptations, both canon-wins calls:                  */
/*   1. pathVerdict is the canon four-tier VerdictKey from             */
/*      @/lib/brand (the reference's 3-tier vocabulary was never       */
/*      adopted; this repo's canon home for verdict meta is            */
/*      @/lib/brand — see lib/planner/verdict-ui.ts). Display labels   */
/*      resolve through VERDICT_META — e.g. ALMOST_THERE renders       */
/*      "ALMOST THERE" — never hardcoded.                              */
/*   2. The not-advice guardrail also catches the direct "should I     */
/*      buy" question (the reference only guarded product/rate/trade   */
/*      phrasings, letting "should I buy" fall through to the          */
/*      reflective default). The companion never advises on the        */
/*      decision itself — it redirects to whether YOUR numbers         */
/*      support it.                                                    */
/* ------------------------------------------------------------------ */
...
/** Canon display label for a verdict key, resolved via @/lib/brand meta. */
function verdictLabel(verdict: VerdictKey): string {
...
function readiness(ctx: CompanionFinanceContext): string {
  ...
  if (ctx.pathVerdict === "READY") {
    parts.push(
      `Your live finance path reads ${verdictLabel("READY")}. Cash flow ${fmtMoney(ctx.netCashFlow)}, runway ${runway}, DTI ${ctx.dti.toFixed(0)}%, savings rate ${ctx.savingsRate.toFixed(0)}%.`,
    );
    parts.push(
...
function pathTalk(ctx: CompanionFinanceContext): string {
  ...
  const pct =
    ctx.pathCompletionPct != null ? ` ${ctx.pathCompletionPct}% of steps complete.` : "";
  return (
    `Path verdict: ${verdictLabel(ctx.pathVerdict)}.${pct} ` +
...
export function buildCompanionReply(
  ...
  // Default reflective with live numbers, keyed to the weakest gauge
  const weak =
