...
import { addDaysISO, daysUntil } from "./derived";
...
export function buildRunwaySeries(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const step = open.filter((b) => b.dueDate === dateISO).reduce((s, b) => s + b.amount, 0);
    deducted += step;
    // Overdue / pre-month bills deduct immediately on day 1.
    if (i === 0) {
      deducted += open.filter((b) => b.dueDate < start).reduce((s, b) => s + b.amount, 0);
    }
    const rollup = dayRollup(dateISO, bills, transactions);
    points.push({
      dateISO,
...
export function dayMatchesFilter(rollup: DayRollup, filter: CalendarFilter): boolean {
  switch (filter) {
    ...
    case "all":
    default:
      return rollup.billItems.length > 0 || rollup.in > 0 || rollup.out > 0;
  }
}
...
export function buildUpcoming(bills: Bill[], todayISO: string): UpcomingItem[] {
  return openBills(bills)
    ...
    .map((bill) => {
      ...
      const label =
        daysAway === 0 ? "due today" : daysAway > 0 ? `in ${daysAway}d` : `${-daysAway}d overdue`;
