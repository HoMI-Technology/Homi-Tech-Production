...
describe("middleware locale routing (next-intl composition)", () => {
  it("redirects /es-prefixed protected routes to the Spanish sign-in", async () => {
    const res = await middleware(req("/es/dashboard"));
    expect(res.status).toBe(307);
    const location = res.headers.get("location") ?? "";
    expect(location).toContain("/es/auth/sign-in");
    expect(location).toContain("next=%2Fes%2Fdashboard");
  });

  it("serves /es public routes in place (the prefix IS the locale segment)", async () => {
    const res = await middleware(req("/es/pricing"));
    expect(res.status).toBe(200);
    expect(res.headers.get("location")).toBeNull();
    // No rewrite needed: /es/pricing already maps onto app/[locale]/ directly.
    expect(res.headers.get("x-middleware-rewrite")).toBeNull();
  });

  it("rewrites unprefixed routes to the default locale", async () => {
    const res = await middleware(req("/pricing"));
    expect(res.status).toBe(200);
    expect(res.headers.get("x-middleware-rewrite")).toContain("/en/pricing");
  });

  it("redirects unprefixed routes to /es when NEXT_LOCALE=es", async () => {
    const request = req("/pricing");
    request.cookies.set("NEXT_LOCALE", "es");
    const res = await middleware(request);
    expect(res.status).toBe(307);
    expect(res.headers.get("location")).toContain("/es/pricing");
  });

  it("serves /es pages directly without redirect", async () => {
    const res = await middleware(req("/es"));
    expect(res.status).toBe(200);
    expect(res.headers.get("location")).toBeNull();
  });

  it("never locale-rewrites API routes", async () => {
    const res = await middleware(req("/api/csp-report"));
    expect(res.status).toBe(200);
    expect(res.headers.get("x-middleware-rewrite")).toBeNull();
  });

  it("never locale-rewrites the auth callback or sign-out handlers", async () => {
    for (const path of ["/auth/callback", "/auth/sign-out"]) {
