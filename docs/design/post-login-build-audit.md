...
date: 2026-08-17
revision: 7 — 2026-08-20 F8 guest-only /results shell + drop onboarding_completed write
status: design — phases 00–07 mostly landed; /results file kept for LHCI guest empty; physical route deletion still open
...
| Collapse `/plan` + `/results` | F8 / phase 07 | **Partial** — `/plan` read-only; `/results` guest empty / LHCI only + signed-in → Home; off chrome/palette; physical file deletion still open (LHCI URL) |
...
| 07 | Collapse `/plan` and `/results` into the Build; re-point nav catalog + parity test | F8 | Medium — routing | **Partial** — `/plan` read-only; Assess → `/dashboard`; `/results` guest empty / LHCI shell + signed-in → Home; physical route deletion still open |
