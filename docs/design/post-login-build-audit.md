...
| Collapse `/plan` + `/results` | F8 / phase 07 | **Partial** — `/plan` read-only; `/results` reveal-only (no operate); full route deletion still open |
...
| 07 | Collapse `/plan` and `/results` into the Build; re-point nav catalog + parity test | F8 | Medium — routing | **Partial** — `/plan` read-only + palette; Results operate removed; full `/results` merge still open |
