...
- **LAUNCH.md** — 2026-07-23 launch checklist, superseded by `GO-LIVE-CHECKLIST.md` (root), which is THE launch doc. Known-stale bits here: the "PR #81 i18n sweep" item (i18n was removed in #125), `RECEIPT_SIGNING_KEY` (dead name — the real var is `RECEIPT_SIGNING_SECRET`), and "apply 00001–00033 via `supabase db push`" (do not — see `docs/ops/MIGRATIONS-SSOT.md`). Two further contradictions vs GO-LIVE: its Stripe webhook event list (checkout.session.completed / invoice.payment_succeeded / charge.refunded) does not match the four events the route actually handles, and its SMTP guidance — the Resend SMTP username is the literal string `resend` with the API key as password, not a per-account username.
- **pr-wave2.md** — leftover Wave 2 PR body (`feat/decision-os-coherence`). Not a living checklist; archived 2026-09-11 (nightly audit U).
