...
- **`app/[locale]/(product)/team/page.tsx` + `supabase/migrations/00042_org_assessment_deidentify.sql`** — org-member assessment reads now use a de-identified projection; `inputs`/`sub_scores`/`insights` are no longer exposed to teammates.
...
- **`supabase/migrations/00043_tools_overlay_sync.sql`** — new `user_tools_overlay` table with owner-only RLS.
...
- `supabase/migrations/00042_org_assessment_deidentify.sql`
- `supabase/migrations/00043_tools_overlay_sync.sql`

## Pre-merge checklist

- [ ] Human review of `00042` and `00043` migrations.
