...
export default function RentVsBuyPage() {
  ...
  const data = useMemo(
    () => simulate(rent, price, rate, appreciation, years),
    [rent, price, rate, appreciation, years],
  );
  ...
  const rentPoints = [
    `${scaleX(0)},${scaleY(0)}`,
    ...data.map((d) => `${scaleX(d.year)},${scaleY(d.rentCost)}`),
  ].join(" ");
  const buyPoints = [
    `${scaleX(0)},${scaleY(0)}`,
    ...data.map((d) => `${scaleX(d.year)},${scaleY(d.buyCost)}`),
  ].join(" ");
        ...
        <div className="space-y-6">
          <div className="glass p-6">
            ...
            <div className="mt-3 flex gap-6 text-xs text-dim">
              <span className="flex items-center gap-2">
                <span className="inline-block h-2 w-2 rounded-full bg-yellow" /> Renting
                (cumulative)
              </span>
              <span className="flex items-center gap-2">
                <span className="inline-block h-2 w-2 rounded-full bg-cyan" /> Buying (net of
                equity)
              </span>
            </div>
          </div>
            ...
            <div className="glass p-6">
              <div className="grid grid-cols-2 gap-4 text-center">
                <div>
                  ...
                  <p className="score-numeral mt-1 text-lg font-bold text-yellow">
                    {formatCurrency(finalYear.rentCost)}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-dim">Buying, {years}yr net cost</p>
                  <p className="score-numeral mt-1 text-lg font-bold text-cyan">
                    {formatCurrency(finalYear.buyCost)}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="glass p-6">
            ...
            <p className="mt-2 text-sm leading-relaxed text-dim">
              At this horizon, {buyIsCheaper ? "buying" : "renting"} looks cheaper on paper — but
              this assumes steady appreciation and ignores the risk of needing to sell in a down
              market, closing costs, and how a mortgage changes your flexibility to move. The answer
              depends on timing, not just math: how long you'll actually stay, and how much
              certainty you need, matter as much as the numbers above.
            </p>
          </div>
