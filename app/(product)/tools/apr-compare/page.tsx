...
function AprComparePageInner() {
  ...
  const readiness = useHousingReadinessImpact(
    winning ? { monthlyObligation: winning.monthly, upfrontCost: winning.upfrontCost } : null,
  );
        ...
        <div className="space-y-6 lg:col-span-2">
          <div className="grid gap-4 sm:grid-cols-3">
            {results.map((r, i) => (
              <div key={i} className={`glass p-5 ${i === best ? "panel-focus" : ""}`}>
                ...
                <p className="score-numeral text-2xl font-bold text-cyan">
                  {formatPercent(r.apr, 3)}
                </p>
                <div className="mt-4 space-y-2 text-xs text-dim">
                  <div className="flex justify-between">
                    <span>Note rate</span>
                    <span className="score-numeral text-light">{formatPercent(r.rate, 3)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Monthly P&amp;I</span>
                    <span className="score-numeral text-light">{formatCurrency(r.monthly)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span>Total over term</span>
                    <span className="score-numeral text-light">{formatCurrency(r.totalCost)}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="glass p-6">
            ...
            <p className="mt-2 text-sm leading-relaxed text-dim">
              A lender can advertise a low note rate and quietly charge points and fees to get
              there. The APR folds those upfront costs back into an equivalent rate, so a 5.75% loan
              with two points can end up <em>more</em> expensive than a 6.25% loan with none. Rank
              by the true APR (and by how long you&rsquo;ll actually keep the loan) — not the number
              on the flyer. Educational math, not a quote.
            </p>
          </div>
