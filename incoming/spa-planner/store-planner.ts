...
/**
 * Build a PathSnapshot from live budget state. Exported so the adapter
 * can be exercised purely (node, no DOM) — regeneratePath is a thin
 * `set({ path: buildPlannerPathSnapshot(get()) })` wrapper.
 */
export function buildPlannerPathSnapshot(
  ...
  const result = scoreFromBudget({
    ...
    readinessProfile: state.readinessProfile,
    debts: state.debts,
  })
  const assessment = assessmentFromScore(result)
  ...
  // Project canon ReadinessPath → PathSnapshot: the internal route href
  // is dropped (the planner has no router surface for it); everything
  // else carries over one-for-one, including canon verdict + score.
  return {
    id: path.id,
    ...
    steps: path.steps.map((step) => {
      /* Canon steps carry an app-route href; the planner snapshot drops it. */
...
export const usePlannerStore = create<PlannerStore>()(
  persist(
    (set, get) => ({
      ...
      regeneratePath: () => {
        // Canon binding-constraint engine (lib/path.ts) driven by the
        // canon scorer — see buildPlannerPathSnapshot. Completion still
        // flows exclusively through completePathStep / the closed loop;
        // a regenerate rebuilds the sequence from live numbers only.
        set({ path: buildPlannerPathSnapshot(get()) })
      },
