 ...
 *
 * Grounded in protective honesty (not gamified guilt):
...
function todayISO(fallback?: string): string {
...
export function deriveBehaviorNudges(input: NudgeEngineInput): BehaviorNudge[] {
  ...
  if (input.assessment && input.assessment.hardStops.length > 0) {
    const hs = input.assessment.hardStops[0];
    nudges.push({
      id: "nudge-protect-hardstop",
      kind: "protect_decision",
      priority: 1,
      title: "Protect the decision first",
      body: `${hs.message} One protective move beats three optimistic ones.`,
  ...
  if (
    input.assessment &&
    input.assessment.emotional.partnerAlignment > 0 &&
    input.assessment.emotional.partnerAlignment < 4.5
  ) {
    nudges.push({
