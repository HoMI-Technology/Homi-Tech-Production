/**
 * Mortgage / rent-vs-buy lens — deterministic CFM housing math.
 */
...
export interface HousingLensInput {
  ...
  netCashFlow: number;
  /** Expected home appreciation annual % */
  appreciationPct?: number;
  /** Opportunity cost of down payment annual % */
  opportunityPct?: number;
  /** Years to project cumulative cost */
  horizonYears?: number;
}
...
export function computeHousingLens(input: HousingLensInput): HousingLensResult {
  ...
  const monthlyPi = principalAndInterest(
    loanAmount,
    input.ratePct,
    input.termYears,
  );
  ...
  const opportunityCost =
    input.downPaymentSaved * (Math.pow(1 + opp, horizon) - 1);
  ...
  let breakEvenYears: number | null = null;
  {
    ...
    for (let m = 1; m <= 30 * 12; m++) {
      ...
      bal -= prin;
      const hv =
        input.targetPrice * Math.pow(1 + appr, m / 12) - bal;
  ...
  if (monthlyDelta > 0) {
    notes.push(
      `Buy costs ${fmt(monthlyDelta)} more per month than current rent.`,
    );
  } else {
    notes.push(
  ...
  else if (housingRatioPct > 36 || monthlyDelta > input.netCashFlow)
    verdict = "buy_stretch";
