/**
 * Canonical Financial Model (CFM) — source-labeled view of planner numbers.
 * Adapted from production lib/tools/cfm.ts for the local-first budget store.
 */
...
export function deriveCfm(
  ...
  return {
    ...
    horizon: {
      ...
      expectedReturnPct: sourced(
        finance.expectedReturnPct ?? 7,
        finance.expectedReturnPct != null ? "self-reported" : "derived",
      ),
      volatilityPct: sourced(
        finance.volatilityPct ?? 12,
        finance.volatilityPct != null ? "self-reported" : "derived",
      ),
    },
    derived: {
