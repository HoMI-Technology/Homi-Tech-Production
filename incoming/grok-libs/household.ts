/**
 * Dual household readiness — min(joint) doctrine from production.
 */
...
export function computeDualHouseholdScore(
  ...
  if (jointHardStops.length > 0) {
    summary = `Household DO NOT PROCEED — ${jointHardStops.length} protective hard-stop(s) across members. Clear gates before a joint move.`;
  } else if (!verdictAligned) {
    summary = `Verdicts differ (${a.label}: ${a.result.verdict}, ${b.label}: ${b.result.verdict}). Joint score ${jointScore} follows the weaker readiness.`;
  } else if (scoreGap >= 15) {
