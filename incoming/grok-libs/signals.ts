/**
 * Proactive signals from live planner state —
 * cash flow, bills, runway, path, score pillars, and stress algorithms.
 */
...
export function derivePlannerSignals(input: SignalEngineInput): PlannerSignal[] {
  ...
  if (input.assessment && input.assessment.hardStops.length > 0) {
    const first = input.assessment.hardStops[0];
    ...
    signals.push({
      ...
      body: `${first.message} Not yet is not no — it is protection.`,
  ...
  const overdue = openBills.filter(
    (b) => b.status === "overdue" || daysUntil(b.dueDate) < 0,
  );
  if (overdue.length > 0) {
    ...
    if (dueSoon.length > 0) {
      const d = daysUntil(dueSoon[0].dueDate);
      signals.push({
        id: "bills-due-soon",
        ...
        title:
          d === 0
            ? `${dueSoon[0].name} is due today`
            : `Bill due in ${d} day${d === 1 ? "" : "s"}`,
  ...
  if (input.assessment) {
    const { financial, emotional, timing } = input.assessment;
    const pillars = [
    ...
    if (
      input.assessment.verdict === "READY" &&
