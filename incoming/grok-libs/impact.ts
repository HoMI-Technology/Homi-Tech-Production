 ...
 *
 * Descriptions are operator-grade: what moved, why it matters, what to do next.
 * Protective honesty — never celebrate a drop as "failure"; never inflate noise.
 * Path steps: prefer progress framing when score is flat.
 */

import type { AssessmentResult, Verdict } from "./scoring";
import { VERDICT_META } from "./scoring";
...
function pillarNarrative(
  ...
  const deltas = {
    financial: round1(after.financial.total - before.financial.total),
    emotional: round1(after.emotional.total - before.emotional.total),
    timing: round1(after.timing.total - before.timing.total),
  };
  const ranked: Array<[string, number]> = [
    ["Financial Reality", deltas.financial],
    ["Emotional Truth", deltas.emotional],
    ["Perfect Timing", deltas.timing],
  ];
...
export function buildScoreImpact(
  ...
  const beforeCodes = new Set(before.hardStops.map((h) => h.code));
  const afterCodes = new Set(after.hardStops.map((h) => h.code));
  ...
  if (hardStopsCleared > 0 && delta >= 0) {
    ...
    headline = `Verdict advanced · ${fromV.short} → ${toV.short}`;
  } else if (verdictShift && delta < 0) {
    headline = `Verdict tightened · ${fromV.short} → ${toV.short}`;
  } else if (delta > 0.5) {
    headline =
...
export function impactToSnapshot(impact: ScoreImpact) {
  return {
    id: impact.id,
